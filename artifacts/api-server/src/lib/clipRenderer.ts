...
export interface ClipSegmentInput {
  ...
  /**
   * BCP-47 language tag of the translation text ("en" | "ur").
   * Used only for font selection in the rendered frame — not for TTS.
   */
...
async function renderClipSvg(params: {
  ...
  // Urdu Nastaliq words are wider — use tighter char limit per line
  const translationLines = wrapText(params.translation, isUrdu ? 30 : 50, 10);
  const arabicFontSize = arabicLines.length > 5 ? 52 : arabicLines.length > 3 ? 58 : 64;
  const arabicLineHeight = arabicFontSize + 22; // Amiri Quran needs extra leading
  const translationFontSize = isUrdu
    ? (translationLines.length > 6 ? 32 : 36)
    : (translationLines.length > 6 ? 30 : 34);
  const translationLineHeight = isUrdu
    ? translationFontSize + 24  // Nastaliq ascenders/descenders need more room
    : translationFontSize + 14;
  const translationFontFamily = isUrdu ? "Noto Nastaliq Urdu, serif" : "Inter, Arial, sans-serif";
  const translationDirection = isUrdu ? "rtl" : "ltr";
